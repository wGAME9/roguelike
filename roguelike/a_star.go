...
func (as AStar) GetPath(level level, start *position, end *position) []position {
	...
	for {
		...
		if currentNode.Position.Y > 0 {
			tile := level.Tiles[level.getIndexFromCoords(currentNode.Position.X, currentNode.Position.Y-1)]
			if tile.TypeOfTile != WALL {
		...
		if currentNode.Position.Y < numTilesY {
			tile := level.Tiles[level.getIndexFromCoords(currentNode.Position.X, currentNode.Position.Y+1)]
			if tile.TypeOfTile != WALL {
				//The location is in the map bounds and is walkable
		...
		if currentNode.Position.X > 0 {
			tile := level.Tiles[level.getIndexFromCoords(currentNode.Position.X-1, currentNode.Position.Y)]
			if tile.TypeOfTile != WALL {
				//The location is in the map bounds and is walkable
		...
		if currentNode.Position.X < numTilesX {
			tile := level.Tiles[level.getIndexFromCoords(currentNode.Position.X+1, currentNode.Position.Y)]
			if tile.TypeOfTile != WALL {
				//The location is in the map bounds and is walkable
